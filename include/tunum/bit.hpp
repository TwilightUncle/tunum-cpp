...
namespace tunum
{
    ...
    // 右循環シフト
    inline constexpr impl::rotr_cpo rotr{};

    // 左から連続した0のビットを数える
    inline constexpr impl::countl_zero_cpo countl_zero{};

    // 右から連続し0のビットを数える
    inline constexpr impl::countr_zero_cpo countr_zero{};

    // 左から連続した1のビットを数える
    inline constexpr impl::countl_one_cpo countl_one{};

    // 右から連続した1のビットを数える
    inline constexpr impl::countr_one_cpo countr_one{};

    // 立っているビット数をカウント
    inline constexpr impl::popcount_cpo popcount{};

    // 一つだけbitが立っているか判定
    inline constexpr impl::has_single_bit_cpo has_single_bit{};

    // 値を表現するのに必要なビット幅を取得
    inline constexpr impl::bit_width_cpo bit_width{};

    // 整数値を2の累乗に押し上げる
    inline constexpr impl::bit_ceil_cpo bit_ceil{};

    // 整数を2の累乗に押し下げる
    inline constexpr impl::bit_floor_cpo bit_floor{};
}
